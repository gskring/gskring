...
let from_member_list = function
  ...
  | x :: xs ->
    ...
    let rec aux snd pred acc = function
      ...
      | [ curr ] ->
        let latest =
          { pred = curr; curr = first; succ = Option.value ~default:curr snd }
        in
...
let fold f start chain =
  List.fold_left (fun acc { pred; curr; succ } -> f acc ~pred ~curr ~succ) start chain
;;
...
let as_author chain id =
  ...
  |> Option.fold ~none:unknow_author (* Should never happen *) ~some:Member.as_author
;;
