  ...
  Yocaml.Task.from_effect (fun () ->
    ...
    let* files =
      read_directory
        ~on:`Source
        ~only:`Files
        ~where:(Yocaml.Path.has_extension "yaml")
        path
    in
    let+ articles =
      List.traverse
        (fun file ->
           Yocaml_yaml.Eff.read_file_as_metadata (module Article) ~on:`Source file)
        files
    in
    limit
...
let atom chain path =
  ...
  fetch chain path
  >>> Atom.from ~updated ~title ~subtitle ~id ~links ~authors (Article.to_atom chain)
;;
